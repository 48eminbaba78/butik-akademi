...
// önceden ikisi de aynı "HİKAYE" promptunu kullanıyordu, Feed Post sekmesinden
// üretilen içerik hep story boyutu/diliyle çıkıyordu ("post değil story
// oluşturuyor" hatası). Artık type'a göre ayrı boyut + ayrı çerçeveleme var.
function svgRules(type) {
  ...
  return '\n## SVG GÖRSEL KURALLARI (ÇOK ÖNEMLİ)\n- Boyut: ' + dims + '. width/height yazma, sadece viewBox.\n- Arka plan: koyu (#0d0d0f - #1a1a1e arası), degrade kullanabilirsin.\n- SADECE şu fontları kullan: font-family="Arial, sans-serif" (sunucuda başka font yok).\n- Harici görsel, harici font, <image href="http...">, <script> KESİNLİKLE YOK. Saf vektör.\n- Metinler BÜYÜK ve okunaklı olsun (başlık 70-110px, gövde 40-55px). Telefonda okunacak.\n- METİNLER ÜST ÜSTE BİNMESİN (sık hata): aynı sütundaki bir sonraki <text>\'in\n  y\'si, öncekinin y\'si + öncekinin font-size\'ı (büyük gösterge rakamlarında\n  1.2 katı) kadar aşağıda olsun. Örn: y="480" font-size="320" "0" varsa,\n  altındaki etiket en erken y="850" olmalı (70px fark değil).\n- TÜRKÇE KARAKTERLER SVG <text> İÇİNDE DE ZORUNLU — burası caption\'dan farklı değil.\n  ç, ğ, ı, İ, ö, ş, ü, Ç, Ğ, Ö, Ş, Ü harflerini ASCII karşılığına (c, g, i, o, s, u)\n  ASLA indirgeme. SVG/XML UTF-8\'i sorunsuz destekler, escape/encoding riski YOK —\n  bu yüzden çekinip sadeleştirme. Önceki üretimlerde şu hata sık tekrarlandı, TEKRARLAMA:\n  YANLIŞ: "Simdi", "Coz", "ogrenme", "kac saat", "gun ucretsiz", "yanlissin", "gercek"\n  DOĞRU:  "Şimdi", "Çöz", "öğrenme", "kaç saat", "gün ücretsiz", "yanlışsın", "gerçek"\n  Her <text> yazmadan önce içindeki her kelimeyi doğru Türkçe yazımıyla iki kez kontrol et.\n- Üstte veya altta "ROSTRUM AKADEMİ" logosu metin olarak (ROSTRUM normal, AKADEMİ bold, altın renk).\n' + uiWarning + '\n- Marka renklerini kullan: altın #f0a500 (ana vurgu), mor #c084fc, yeşil #3ecf8e, mavi #60b4ff.\n- Kompozisyon: tek net mesaj, bol boşluk, 3-5 metin bloğunu geçme. Basit geometrik şekiller/çizgiler serbest.\n- Metin satırları SVG\'de otomatik sarmaz: uzun cümleleri kendin birden çok <text> elemanına böl, her satır max ~20 karakter (büyük punto) / ~35 karakter (küçük punto).\n';
}
...
export async function generateForDate(dateStr, opts) {
  opts = opts || {};
  var client = sb();
  // post_type kolonu var ama önceden hiç yazılmıyordu — Feed Post sekmesinden
  // üretilen içerik hep varsayılan (story) olarak kaydolup kendi kuyruğunda
  // hiç görünmüyordu. type=story ile aynı tarihte iki ayrı içerik (biri story
  // biri feed) bir arada var olabildiği için "var mı" kontrolü de tipe göre.
  ...
  var recent = await client
    ...
    .limit(7);
  // .slice(0,80) UTF-16 code unit sınırında kesiyordu — emoji gibi surrogate-pair
  // karakterler tam ortadan bölünüp Anthropic'e "no low surrogate" JSON hatası
  // verdiriyordu. Array.from ile kod noktası bazında (bölünmeden) kesiyoruz.
  ...
  var resp = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({
      // 4000 bazen yetersiz kalıp AI'nin cevabı (JSON'ı hiç tamamlayamadan)
      // ortada kesilmesine yol açıyordu — "AI ciktisi JSON olarak
      // ayristirilamadi" hatasının bir kısmı bu yüzdendi. 6000'e çıkarıldı;
      // daha da yükseltmek Vercel'in 60sn fonksiyon süresi sınırına
      // (vercel.json) çarpma riskini artırır, o yüzden ölçülü tutuldu.
      model: process.env.ANTHROPIC_MODEL || 'claude-sonnet-5',
      max_tokens: 6000,
      system: BRAND_BRAIN,
      messages: [{ role: 'user', content: buildPrompt(dateStr, theme, recentSummaries, yksInfo, opts.idea || '', type) }],
    }),
  });
  ...
  var parsed;
  try {
    if (!clean) throw new Error('AI bos yanit dondu (stop_reason: ' + data.stop_reason + ')');
    parsed = JSON.parse(clean);
  } catch(e) {
    // stop_reason 'max_tokens' ise yanit ortadan kesilmis demektir — bu durumda
    // JSON hep bozuk cikar (kapanis parantezi/tirnagi hic gelmemistir).
    var diag = 'AI ciktisi JSON olarak ayristirilamadi (stop_reason: ' + data.stop_reason + ', uzunluk: ' + clean.length + ', hata: ' + e.message + ').';
    var row = {
...
async function publishToInstagram(client, pngUrl, postType) {
...
export async function publishDue(opts = {}) {
  ...
  for (var post of (due.data || [])) {
    try {
      ...
      var mediaId = await publishToInstagram(client, pngUrl, post.post_type);

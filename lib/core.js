...
function buildPrompt(dateStr, theme, recentSummaries, yksInfo, idea, type, archetype) {
  var isFeed = type === 'feed';
...
export async function generateForDate(dateStr, opts) {
  ...
  // claude-3-5-sonnet-20241022 Anthropic tarafından 28 Ekim 2025'te emekli edildi
  // (404 not_found_error) — güncel karşılığı claude-sonnet-5.
    ...
    try {
      var resp = await fetch('https://api.anthropic.com/v1/messages', {
        ...
        body: JSON.stringify({
          model: modelName,
          max_tokens: 4096,
          // claude-sonnet-5, thinking parametresi belirtilmezse VARSAYILAN olarak
          // adaptive thinking ile çalışıyor (eski Claude 3.5 modelinde bu davranış
          // yoktu) — düşünme token'ları max_tokens bütçesinden düşüp JSON/SVG
          // çıktısının ortasında kesilmesine (unterminated string) yol açıyordu.
          // Bu basit içerik üretimi görevi için düşünmeye gerek yok.
  ...
  // Bilinen hata kalıplarına (yanlış deneme süresi, uydurma fiyat) takılmayan
  // içerik otomatik onaylanır — kurucu her gün manuel onaylamak zorunda
  // kalmadan yayın akışı devam eder. Şüpheli olan taslak halinde kalır.
  var row = {
    post_date: dateStr,
    theme: theme.key,
    post_type: type,
    caption: caption,
    svg: parsed.svg,
    status: riskNote ? 'draft' : 'approved',
    error_msg: riskNote ? ('Otomatik onay atlandı: ' + riskNote) : null,
    publish_at: trToUtc(dateStr, theme.hourTR),
    updated_at: new Date().toISOString(),
  };

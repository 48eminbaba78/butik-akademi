...
export async function generateForDate(dateStr, opts) {
  ...
  var modelName = (process.env.ANTHROPIC_MODEL || 'claude-3-5-haiku-20241022').trim();
  ...
  try {
    ...
    if (err.name === 'AbortError') {
      throw new Error('AI yanıtı 40 saniyelik zaman aşımına uğradı. Lütfen daha kısa bir fikir metni girip tekrar deneyin.');
    }
    throw err;
  } finally {
    clearTimeout(timer);

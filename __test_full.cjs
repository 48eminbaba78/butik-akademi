...
async function logout() {
  try {
    await page.click('#tnUserWrap .tn-user');
    await page.waitForTimeout(400);
    await page.click('.tn-user-menu-item.danger');
...
(async () => {
    ...
    if (await progCard.count() > 0) {
      ...
      if (await addBtn.count() > 0) {
        ...
        if (modalVisible) {
          ...
          const saveBtn = page.locator('#taskModal button[onclick*="saveTask"]');
          await saveBtn.click();
          await page.waitForTimeout(2000);
  ...
  try {
    const resTab = page.locator('.tn-nav-item').filter({ hasText: /kaynak/i }).first();
    if (await resTab.count() > 0) {
      await resTab.click();
      await page.locator('.cr-filter-bar').waitFor({ state: 'visible', timeout: 6000 }).catch(() => {});
  ...
  try {
    await page.click('#tnUserWrap .tn-user');
    await page.waitForTimeout(300);
    await page.click('#tnCoachProfileItem');
    ...
    log('✅', 'PRF-1', 'Profil sekmesi açıldı');
      ...
      // Ad alanı — try multiple selectors inside active view
      const nameField = await page.evaluate(() => {
        const candidates = [
          '.view.active #coachName',
          '.view.active #cpName',
          '.view.active #profileName',
          '.view.active input[name="name"]',
          '.view.active input[placeholder*="ad"]',
          '.view.active #cpSubjects',
          '.view.active input[type="text"]'
        ];
